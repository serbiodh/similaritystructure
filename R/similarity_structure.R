...
#' @param sav Logical. Should the empirical PN-alpha object be saved?
...
#' \describe{
#'   \item{empiric_PNalpha}{Empirical P(N | >alpha) curve.}
...
similarity_structure <- function(n1, n2, N_init, N_fin, num_N, num_repet,
  ...
  # Save data frame with empirical P(N|>alfa) in case the next step fails
  if (sav == T) {

    if (is.null(sav_file) == T) {
      saveRDS(propsig, "empirical_PNalpha.RData")
  ...
  return(list(empiric_PNalpha = propsig,
